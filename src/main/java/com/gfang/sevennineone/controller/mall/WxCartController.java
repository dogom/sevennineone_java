package com.gfang.sevennineone.controller.mall;

import com.gfang.sevennineone.anotation.LoginUser;
import com.gfang.sevennineone.model.po.SnoUserPO;
import com.gfang.sevennineone.service.mall.*;
import com.gfang.sevennineone.system.SystemConfig;
import com.gfang.sevennineone.util.JacksonUtil;
import com.gfang.sevennineone.util.ResponseUtil;
import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;
import com.gfang.sevennineone.model.po.mall.*;
...
import static com.gfang.sevennineone.util.WxResponseCode.GOODS_NO_STOCK;
import static com.gfang.sevennineone.util.WxResponseCode.GOODS_UNSHELVE;
...
public class WxCartController {
     ...
     */
    @GetMapping("index")
    public Object index(@LoginUser SnoUserPO user) {
        if (user == null) {
            return ResponseUtil.unlogin();
        }

        List<LitemallCart> cartList = cartService.queryByUid(user.getId());
     ...
     */
    @PostMapping("add")
    public Object add(@LoginUser SnoUserPO user, @RequestBody LitemallCart cart) {
        if (user == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
...
//        }
        ...
        //判断购物车中是否存在此规格商品
        LitemallCart existCart = cartService.queryExist(goodsId, productId, user.getId());
        if (existCart == null) {
            //取得规格的信息,判断规格库存
     ...
     */
    @PostMapping("fastadd")
    public Object fastadd(@LoginUser SnoUserPO user, @RequestBody LitemallCart cart) {
        if (user == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
...
//        }
     ...
     */
    @PostMapping("checked")
    public Object checked(@LoginUser SnoUserPO user, @RequestBody String body) {
        if (user == null) {
            return ResponseUtil.unlogin();
        }
        if (body == null) {
        ...
        cartService.updateCheck(user.getId(), productIds, isChecked);
        return index(user);
    }
     ...
     */
    @PostMapping("delete")
    public Object delete(@LoginUser SnoUserPO user, @RequestBody String body) {
        if (user == null) {
            return ResponseUtil.unlogin();
        }
        if (body == null) {
    ...
    public Object goodscount(@LoginUser SnoUserPO user) {
        if (user == null) {
     ...
     */
    @GetMapping("checkout")
    public Object checkout(@LoginUser SnoUserPO user, Integer cartId, Integer addressId, Integer couponId, Integer grouponRulesId) {
        if (user == null) {
            return ResponseUtil.unlogin();
        }
        ...
        if (addressId == null || addressId.equals(0)) {
            checkedAddress = addressService.findDefault(user.getId());
            // 如果仍然没有地址，则是没有收获地址
            ...
            checkedAddress = addressService.query(user.getId(), addressId);
            // 如果null, 则报错
            if (checkedAddress == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        if (cartId == null || cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(user.getId());
        } else {
        ...
        List<LitemallCouponUser> couponUserList = couponUserService.queryAll(user.getId());
        for(LitemallCouponUser couponUser : couponUserList){
            LitemallCoupon coupon = couponVerifyService.checkCoupon(user.getId(), couponUser.getCouponId(), checkedGoodsPrice);
            if(coupon == null){
                continue;
            }
        ...
        if (couponId == null || couponId.equals(-1)){
            ...
            LitemallCoupon coupon = couponVerifyService.checkCoupon(user.getId(), couponId, checkedGoodsPrice);
